-- Fix for "null value in column registration_type" error
-- This updates the register_fest_user function to include registration_type = 'solo'
...
BEGIN
  BEGIN
    -- 1. Get Fest Event ID
    -- Try specific ID first, then fallback to any 'fest' type event
    ...
    IF v_fest_event_id IS NULL THEN
       RAISE EXCEPTION 'Fest event not found. Please ensure a "fest" type event exists in the events table.';
    END IF;
    ...
    -- FIXED: Added registration_type = 'solo'
    ...
    ON CONFLICT (profile_id, event_id) DO UPDATE SET
      ...
      registration_type = 'solo', -- Ensure type is set on update too
      updated_at = now()
