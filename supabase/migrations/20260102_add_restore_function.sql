-- Function to restore fest registrations from profiles
CREATE OR REPLACE FUNCTION restore_fest_registrations_from_profiles()
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_proof_url TEXT;
BEGIN
  -- Loop through profiles that have fest registration data
  ...
  LOOP
    ...
    -- Try to find the proof URL from the old registrations table
  ...
  RETURN json_build_object('success', true, 'restored_count', v_count);
END;
$$;
