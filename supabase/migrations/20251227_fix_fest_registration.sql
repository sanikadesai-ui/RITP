...
-- Ensure the RPC function is correct and robust
...
BEGIN
  BEGIN
    -- 1. Get or Create Fest Event ID
    -- First try to find the fixed ID
    ...
    -- If still not found, we MUST raise an error or create it (but creating in a function is risky if permissions missing)
    -- We will assume the INSERT above ran. If not, we fail.
    IF v_fest_event_id IS NULL THEN
       RAISE EXCEPTION 'Fest event not found. Please run the setup SQL.';
    END IF;
    ...
    -- 4. Create Proof Upload Record (for redundancy/audit)
